...
FeederBot.prototype = {
    ...
    attachEvents: function() {
        ...
        bot.client.on('connected', function() {
            ...
            bot.client.spawn("SUPRIX");
			//bot.client.spectate();
			//bot.client.moveTo(-9000, 9000);
            spawnCount++;
        ...
        bot.client.on('myNewBall', function(ball_id) {
            ...
            if (config.verbosityLevel > -1) {
				bot.client.ball_id = ball_id;
                bot.log('New Cell Generated (' + ball_id + ')');
            }
        });

        bot.client.once('leaderBoardUpdate', function(data) {
        ...
        bot.client.on('packetError', function(packet, err, preventCrash) {
           bot.log('Packet error detected for packet: ');
...
var socket = require('socket.io-client')("ws://localhost:8080");
...
socket.on('buf', function(data) {
	 ...
	 a = data.toString().split(",")[0];
	 b = data.toString().split(",")[1];
	 c = data.toString().split(",")[2];
	 d = data.toString().split(",")[3];
	 e = data.toString().split(",")[4];
    ...
    if (data.name == "split") {
        ...
        setTimeout(function() {
			var sd = setInterval(function(){
				...
				if (a != null && b != null && c != null && d != null && d != null) {
